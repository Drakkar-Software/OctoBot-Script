...
async def test_precomputed_vs_iteration_rsi(one_day_btc_usdt_data):
    ...
    async def _pre_compute_update(ctx):
        if run_data["entries"] is None:
            ...
            await obs.plot_indicator(
                ctx, "RSI", times[delta:], rsi_v, run_data["entries"]
            )
    ...
    assert (
        res.report["bot_report"]["end_portfolio"]["binance"]
    ...
    assert (
        res_2.report["bot_report"]["profitability"]
        == res.report["bot_report"]["profitability"]
    )
    assert (
        res_2.report["bot_report"]["end_portfolio"]["binance"]
    ...
    assert (
        res_3.report["bot_report"]["profitability"]
        != res.report["bot_report"]["profitability"]
    )
    assert (
        res_3.report["bot_report"]["end_portfolio"]["binance"]
    ...
    assert (
        res_iteration.report["bot_report"]["profitability"]
        == res_3.report["bot_report"]["profitability"]
    )
